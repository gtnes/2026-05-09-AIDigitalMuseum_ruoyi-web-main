...
async function handleLogout() {
  ...
  // logout 内部会跳 /chat，立即拉回本页保持不离开
  router.replace('/apps-chat');
  userStore.openLoginDialog();
}
</script>

<template>
  <div class="apps-chat-page">
    <!-- 左上角返回按钮 -->
    ...
    <!-- 右上角退出登录按钮：仅登录状态显示 -->
    <button v-if="userStore.token" class="logout-btn" aria-label="退出登录" @click="handleLogout">
      <el-icon :size="18">
        <SwitchButton />
      </el-icon>
    ...
    <!-- 登录弹窗（/apps-chat 需要登录，未登录就地登录不跳转；不可手动关闭，必须登录） -->
    <MobileLoginSheet v-model:visible="userStore.isLoginDialogVisible" :closable="false" />
  </div>
</template>
